-- =====================================================
-- Income Goal Calculator - Complete Database Schema  
-- FOR SUPABASE PROJECT: Income Goal Calculator
-- Run this SQL in: https://app.supabase.com/project/jkwkrtnwdlyxhiqdmbtm/sql
-- =====================================================
...
CREATE TABLE IF NOT EXISTS users (
  ...
  default_activity_role VARCHAR(20) DEFAULT 'broker' CHECK (default_activity_role IN ('broker', 'investor')),
...
-- =====================================================
-- Tracks daily prospecting activities for paid users (WITH DUAL-ROLE SUPPORT)
...
-- USER SESSIONS TABLE
-- =====================================================
-- Optional - for tracking active sessions
...
-- =====================================================
-- Grant premium access to david@davidmonroeccim.com (updated for new account)
-- This will work for any auth.users record with this email
DO $$
DECLARE
    auth_user_id UUID;
BEGIN
    ...
    FROM auth.users 
    WHERE email = 'david@davidmonroeccim.com';
    
    IF auth_user_id IS NOT NULL THEN
        -- Update or insert user profile with premium access
        INSERT INTO users (
            id, 
            email, 
            password_hash, 
            ...
            default_activity_role
        ) VALUES (
            auth_user_id,
            'david@davidmonroeccim.com',
            'supabase_auth',
            'David',
            'Monroe',
            'broker',
            ...
            updated_at = NOW();
    ELSE
        -- If no auth user exists yet, create placeholder that will be updated on first login
        INSERT INTO users (
            email, 
            password_hash, 
            first_name, 
...
-- =====================================================
COMMENT ON TABLE users IS 'User profiles extending Supabase auth with subscription and preference data';
COMMENT ON TABLE user_goals IS 'Stores calculated goals for both broker and investor roles separately';  
COMMENT ON TABLE daily_activities IS 'Daily activity tracking with dual-role support (broker/investor)';
COMMENT ON TABLE leads IS 'Lead capture from free calculator usage';
COMMENT ON TABLE subscription_events IS 'Stripe subscription event tracking';
...
COMMENT ON COLUMN user_goals.user_type IS 'Goal type: broker or investor calculations';
...
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'daily_activities';
-- SELECT * FROM users WHERE email = 'david@davidmonroeccim.com';